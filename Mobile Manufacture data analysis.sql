...
SELECT STATE,CUSTOMER_NAME,YEAR
FROM DIM_LOCATION T1
LEFT JOIN FACT_TRANSACTIONS T2 ON T1.IDLOCATION=T2.IDLOCATION
LEFT JOIN DIM_DATE T3 ON T2.DATE=T3.DATE
LEFT JOIN DIM_CUSTOMER T4 ON T2.IDCUSTOMER=T4.IDCUSTOMER
WHERE YEAR >= 2005


 --2.What state in the US is buying more 'Samsung' cell phones? 

  SELECT TOP 1 STATE,COUNTRY,MANUFACTURER_NAME, COUNT(QUANTITY) AS COUNT FROM DIM_LOCATION T1
 LEFT JOIN FACT_TRANSACTIONS T2 ON T1.IDLOCATION=T2.IDLOCATION
...
--3.	Show the number of transactions for each model per zip code per state.
 ...
 SELECT ZIPCODE, COUNT(IDMODEL) AS TOTAL_TRANSACTIONS, COUNTRY,STATE
FROM DIM_LOCATION T1
 LEFT JOIN FACT_TRANSACTIONS T2 ON T1.IDLOCATION=T2.IDLOCATION
GROUP BY ZIPCODE,COUNTRY, STATE
...
--6.	List the names of the customers and the average amount spent in 2009, where the average is higher than 500
 ...
 SELECT CUSTOMER_NAME,AVG(TotalPrice),YEAR(DATE) AS AVERAGE_AMOUNT
...
SELECT IDMODEL
FROM (SELECT T1.IDMODEL, YEAR(DATE) AS YYYY, COUNT(*) AS COUNT,
...
SELECT  top 1 * from
    ...
    SUM(Quantity )  TOP_SALES1
    FROM Fact_Transactions T1
    LEFT JOIN DIM_Model T2 ON T1.IDModel = T2.IDModel
    LEFT JOIN DIM_MANUFACTURER D2  ON D2.IDManufacturer = T2.IDManufacturer
    Where DATEPART(Year,date)='2009' 
    group by Manufacturer_name, Quantity 
    Order by  SUM(Quantity ) DESC ) as A,
        (SELECT 
    Top 2 Manufacturer_name,
     SUM(Quantity ) TOP_SALES2
    FROM Fact_Transactions T3
    LEFT JOIN DIM_Model T4 ON T3.IDModel = T4.IDModel
    LEFT JOIN DIM_MANUFACTURER DM2  ON DM2.IDManufacturer = T4.IDManufacturer
...
--9.	Find top 100 customers and their average spend, average quantity by each year. Also find the percentage of change in their spend.

SELECT TOP 100 
YEAR(DATE) AS YEAR,T1.IDCUSTOMER AS CUSTOMER_NAME,TOTALPRICE AS TOTAL_AMT,AVG(TOTALPRICE) AS AVG_SPEND,AVG(QUANTITY) AS AVG_QUANTITY
INTO #DATASOURCE
FROM  FACT_TRANSACTIONS T1
INNER JOIN DIM_CUSTOMER T2 ON T1.IDCUSTOMER = T2.IDCUSTOMER
GROUP BY YEAR(DATE), T1.IDCUSTOMER, TOTALPRICE
ORDER BY AVG_SPEND DESC 
SELECT *
      ,AVG_SPEND- LAG(AVG_SPEND, 1, 0) OVER (PARTITION BY CUSTOMER_NAME ORDER BY YEAR)
FROM #DATASOURCE
